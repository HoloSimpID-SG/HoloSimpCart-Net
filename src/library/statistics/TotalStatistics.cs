...
namespace HoloSimpID
{
    ...
    {
        protected static HashSet<double> _t = new() { 0.0215, 0.0887, 0.10, 0.125, 0.25, 0.5, 0.75, 0.875, 0.90, 0.9113, 0.9785 };
        //-+-+-+-+-+-+-+-+
        ...
        /// <summary>
        /// <strong><see href="https://www.scribbr.com/statistics/skewness/">Skewness</see></strong>
        /// <br/> -+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
        ...
        /// <br/> -+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
        /// </summary>
        public double Skewness => _n < 3 ? double.NaN : (_n * _sk * Math.Sqrt(_v / (_n - 1)) / (_v * _v * (_n - 2))) * (_n - 1);
        /// <summary>
        /// <strong><see href="https://www.scribbr.com/statistics/kurtosis/">Kurtosis</see></strong>
        /// <br/> -+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
        ...
        /// <br/> -+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
        /// </summary>
        public double Kurtosis => _n < 4 ? double.NaN : ((double)_n * _n - 1) / ((_n - 2) * (_n - 3)) * (_n * _kr / (_v * _v) - 3 + 6.0 / (_n + 1));

        public double GeometricMean => _n < 1 ? double.NaN : Math.Exp(_g / _n);
        public double HarmonicMean => _n < 1 ? double.NaN : (double)_n / _h;
        ...
        protected virtual void SecondPassLoop(IEnumerable<KeyValuePair<double, uint>> map)
        {
            ...
            foreach (var (value, freq) in map)
            {
                ...
                _et += proportionF * Math.Log2(proportionF);
        ...
        protected virtual void ThirdPassLoop(IEnumerable<KeyValuePair<double, uint>> map)
        {
            foreach (var (value, freq) in map)
            {
                _nO += (value < _fL || value > _fU) ? freq : 0;
            }
        }
        //-+-+-+-+-+-+-+-+
        #endregion
        ...
        {
            var _oMap = FirstPassLoopList(list).OrderBy(x => x.Key);
            SecondPassLoop(_oMap);
            ThirdPassLoop(_oMap);
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static TotalStatistics Calculate<T>(IEnumerable<KeyValuePair<T, uint>> map) where T : IConvertible => new(map.Select(x => new KeyValuePair<double, uint>(x.Key.ToDouble(null), x.Value)));
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static TotalStatistics Calculate<T>(IEnumerable<T> list) where T : IConvertible => new(list.Select(x => x.ToDouble(null)));
    }
}
