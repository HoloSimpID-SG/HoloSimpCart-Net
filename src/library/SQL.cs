...
namespace HoloSimpID
{
    public static partial class MoLibrary
    {
        ...
        /// </summary>
        public static T GetCastedValueOrDefault<T>(this IDataReader reader, string key, Func<object, T> castDefinition,
        ...
        {
            return GetCastedValueOrDefault(reader, key, x => x as string, defaultValue);
        }

        /// <summary>
        ///     <inheritdoc cref="GetCastedValueOrDefault{T}(IDataReader, string, Func{object, T}, T)" />
        /// </summary>
        public static DateTime GetCastedValueOrDefault(this IDataReader reader, string key,
            DateTime defaultValue = default)
        {
            return GetCastedValueOrDefault(reader, key,
                x => x is DateTime dt
                    ? dt
                    : DateTime.Parse(x.ToString(), CultureInfo.CurrentCulture, DateTimeStyles.RoundtripKind),
                defaultValue);
        }
        ...
        {
            return GetCastedValueOrDefault(reader, key, x => Convert.ToUInt32(x), defaultValue);
        }
        ...
        {
            return GetCastedValueOrDefault(reader, key, x => Convert.ToInt32(x), defaultValue);
        }
        ...
        {
            return GetCastedValueOrDefault(reader, key, x => Convert.ToDouble(x), defaultValue);
        }
