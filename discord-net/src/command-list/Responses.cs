...
namespace HoloSimpID {
  ...
  public static partial class CommandConsts {
    public static readonly ImmutableDictionary<
        string, Func<SocketSlashCommand,
                     Task>> Responses = new Dictionary<string, Func<SocketSlashCommand, Task>> {
      // Add the command name like this, followed by comma:
      // Write this exact line: (Though honestly the "command" part can be anything, but let's
      // not fry our brains here)
      ["create-cart"] =
          async command => {
      ...
      ["cpp-hello"] =
          async command => {
            ...
            await command.RespondAsync(test_class.GetRandom().ToString());
            test_class.Dispose();
          },
    }
