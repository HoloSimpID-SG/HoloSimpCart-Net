...
namespace HoloSimpID
{
  public static class UmaScheduler
  {
    ...
    private static readonly IReadOnlyDictionary<TrackType, int> TrackTypeWeights = new Dictionary<
      TrackType,
      int
    >()
    {
    ...
    private static int HashKey(TrackType type, LengthCategories length) =>
      (1 << (int)type) | 1 << ((int)length + 2);

    private static readonly IReadOnlyDictionary<
      int,
      IReadOnlyList<(Locations location, short length, Direction direction, Extra? extra)>
    > RaceTracks = new Dictionary<
      int,
      IReadOnlyList<(Locations location, short length, Direction direction, Extra? extra)>
    >()
    {
