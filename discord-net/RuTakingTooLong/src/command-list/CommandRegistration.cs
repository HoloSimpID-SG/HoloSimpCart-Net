...
#region Commands Validation
    ...
    foreach (KeyValuePair<string, Func<SocketSlashCommand, Task>> respond in CommandConsts
                 .Responses) {
      ...
      try {
        ...
        if (db_ver != null) {
          ...
          if (db_ver.version_hash != command_vcs.version_hash) {
            Console.WriteLine();
            Console.WriteLine(
                $"{command.Name} is out of date. Reregistering.".SetColor(new(255, 128, 80))
            ...
            Console.WriteLine();
            Console.WriteLine(
                $"{command.Name} is up to date, nothing to do.".SetColor(new(0, 128, 255))
                    .Format(RichStringFormatter.kTerminal));
            Console.WriteLine();
          }
          // Mark Command
