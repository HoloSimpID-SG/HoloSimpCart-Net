...
namespace HoloSimpID {
  public partial class Cart {
    ...
    public async Task<IReadOnlyList<CartItems>> TryGetCartItems(
        AppDbContext? db = null) => await TryGetCartItems(this, db);

    public static async Task<IReadOnlyList<CartItems>> TryGetCartItems(
        Cart cart, AppDbContext? db = null) {
      bool localContext   = db == null;
      db                ??= new AppDbContext();

      IReadOnlyList<CartItems> result = await db.CartItems.Where(x => x.cartDex == cart.uDex)
                                            .Include(c => c.Cart)
                                            .Include(c => c.Simp)
                                            .ToListAsync();
    ...
    public static async Task<Cart> OpenCart(Simp owner, string cartName, DateTime? datePlan = null,
        decimal shippingCost = 0m, AppDbContext? db = null) {
      bool localContext   = db == null;
      db                ??= new AppDbContext();
    ...
    public static async Task<Cart?> TryGet(
        int uDex, Expression<Func<Cart, bool>>? predicate = null, AppDbContext? db = null) {
      predicate ??=
          _              => true;
      bool localContext   = db == null;
      db                ??= new AppDbContext();

      Cart? cart = await db.Carts.Where(predicate)
                       .Include(c => c.Owner)
    ...
    public static async Task<Cart?> TryGet(
        string cartName, Expression<Func<Cart, bool>>? predicate = null, AppDbContext? db = null) {
      predicate ??=
          _              => true;
      bool localContext   = db == null;
      db                ??= new AppDbContext();
    ...
    public static async Task<List<Cart>> GetAllCarts(
        Expression<Func<Cart, bool>>? predicate = null, AppDbContext? db = null) {
      predicate ??=
          _              => true;
      bool localContext   = db == null;
      db                ??= new AppDbContext();
    ...
    public static async Task CloseCart(Cart cart, AppDbContext? db = null) {
      ...
      await db.Carts.Where(c => c.uDex == cart.uDex)
    ...
    public async Task<bool> UpsertItem(
        Simp simp, Item item, int quantity = 1, AppDbContext? db = null) {
      return await UpsertItem(this, simp, item, quantity, db);
    }

    public static async Task<bool> UpsertItem(
        Cart cart, Simp simp, Item item, int quantity = 1, AppDbContext? db = null) {
      ...
      CartItems? cartItems = await db.CartItems.SingleOrDefaultAsync(
          x => x.cartDex == cart.uDex && x.simpDex == simp.uDex);
